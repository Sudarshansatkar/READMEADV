...
public class EmpDaoImp implements EmpDao
{
	...
	static {
		try {
			...
			getData = conn.prepareStatement("select * from employee84");
			insert = conn.prepareStatement("insert into employee84 values(?,?,?)");
			delEmp = conn.prepareStatement("delete from employee84 where id=?");
			getById = conn.prepareStatement("select * from employee84 where id=?");
			updateEmp = conn.prepareStatement("update employee84 set name=? , sal=? where id = ?");
